  ...
  describe "run/1" do
    test "returns a struct when the params are valid" do
      ...
      params = %{
        ...
        ends_at: ~U[2020-07-30 20:59:59Z],
      ...
      assert election.starts_at == ~U[2020-07-01 11:00:00Z]
      assert election.ends_at == ~U[2020-07-30 20:59:59Z]
    ...
    test "returns error when starts_at is greater than ends_at" do
      ...
      params = %{
        ...
        cover: "url",
        notice: "url",
        starts_at: ~U[2020-07-11 11:00:00Z],
        ends_at: ~U[2020-07-10 20:59:59Z],
        created_by_id: admin.id
      }
      assert {:error, %Ecto.Changeset{} = changeset} = CreateElection.run(params)
